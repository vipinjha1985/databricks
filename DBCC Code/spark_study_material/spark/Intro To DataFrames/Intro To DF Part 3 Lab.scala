...
// MAGIC 
// MAGIC This exercise does not have a test cell to verify a correct solution. However, the initial data set read from the source should contain 103,000 records, and the de-duplicated result should have 100,000 records.
...
val exists = dbutils.fs.ls(destFile+"/_SUCCESS").size >= 1

val finalDF = spark.read.parquet(destFile)
val finalCount = finalDF.count()

clearYourResults()
validateYourAnswer("01 Parquet File Exists", 646192812, exists)
